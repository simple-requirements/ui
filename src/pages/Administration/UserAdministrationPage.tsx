...
export function UserAdministrationPage() {
  ...
  const selectedPresence =
    selectedUser === undefined
      ? undefined
      : presenceByUserId.get(selectedUser.id);
  ...
  async function openRoleDialog(
    ...
    if (loggedIn) {
      showToastMessage(
        toastMessages.userRoleChangeBlockedByLogin(user.displayName),
      );
      return;
    }
  ...
  async function closeRoleDialog(state = roleDialog): Promise<void> {
    ...
    if (state.restoreActiveStatus) {
      try {
        await administration.changeStatusAsync({
          user: state.user,
          status: "active",
          notify: false,
        });
  ...
  async function saveRole(role: AccountRole): Promise<void> {
    ...
    try {
      const updatedUser = await administration.changeRoleAsync({
  ...
  useEffect(() => {
    ...
    const selfDeactivationBlocked =
      targetStatus === "deactivated" && selectedUser.id === currentUserId;
    const activationBlocked =
      targetStatus === "active" && !canActivate(selectedUser);

    setAdministratorUserActionContext({
      label:
        targetStatus === "active" ? "Activate account" : "Deactivate account",
      disabled:
      ...
      >
        {userId === undefined ? (
          <UserList
            ...
            onOpenRoleDialog={(user, loggedIn) =>
              void openRoleDialog(user, loggedIn)
            }
