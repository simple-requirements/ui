...
const projectRoleOrder: readonly ProjectRole[] = [
...
function membershipRow(page: Page, displayName: string) {
  return page.getByRole("row", {
    name: new RegExp(escapeRegExp(displayName), "u"),
  });
}
...
async function selectMembershipAdministrationProject(page: Page): Promise<void> {
  ...
  await expect(
    page.getByRole("region", { name: requireContext().project.name }),
...
async function expectBackendMembershipRoles(
  displayName: string,
  ...
  await expect
    .poll(
      ...
      {
        message: `${displayName} should have project roles ${expectedRoles.join(", ")}`,
        timeout: 10_000,
      },
    )
...
async function expectBackendMembershipRemoved(displayName: string): Promise<void> {
  ...
  await expect
    .poll(
      ...
      {
        message: `${displayName} should no longer have a project membership`,
        timeout: 10_000,
      },
    )
    .toBe(false);
...
async function expectVisibleMembershipRoles(
  page: Page,
  displayName: string,
...
When(
  ...
  async ({ page }, displayName: string, roles: string) => {
    ...
    const membership = await setProjectMembership(
...
When(
  ...
  async ({ page }, displayName: string) => {
    await removeProjectMembership(
      requireContext().project.id,
      userByDisplayName(displayName).id,
    );
    await reopenMembershipAdministration(page);
