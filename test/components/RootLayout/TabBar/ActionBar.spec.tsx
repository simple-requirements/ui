...
const mocks = vi.hoisted(() => ({ markProjectRequirementObsoleteRequest: vi.fn(), setRequirementKey: vi.fn() }));

vi.mock('@/api/requirementsApi', async (importOriginal) => {
    const actual = await importOriginal<typeof RequirementsApiModule>();

    return { ...actual, markProjectRequirementObsoleteRequest: mocks.markProjectRequirementObsoleteRequest };
});
...
function renderActionBar(initialRoute = '/'): ReturnType<typeof render> {
    ...
    const router = createMemoryRouter(
        [
            ...
            { path: '/admin/users/:userId?', element, handle: { actionBar: 'administratorUsers' } },
            { path: '/admin/projects/:projectId?', element, handle: { actionBar: 'administratorProjects' } },
            {
...
describe('ActionBar', () => {
    describe('renders', () => {
        ...
        it('shows Create, Edit, and Review in this order on draft requirement detail routes.', () => {
            ...
            expect(createButton.compareDocumentPosition(editButton)).toBe(Node.DOCUMENT_POSITION_FOLLOWING);
        ...
        it('shows project administration actions for a selected Administrator project', () => {
            ...
            expect(screen.getByRole('button', { name: 'Add membership' })).toBeInTheDocument();
        ...
        it('shows the selected-user status action in the Administrator ActionBar', () => {
            actionBarStore.setState((state) => ({
                ...state,
                administratorUserActionContext: { label: 'Deactivate account', disabled: false },
            }));
    ...
    it('requires a reason and marks an approved requirement obsolete.', async () => {
        ...
        await user.type(screen.getByRole('textbox', { name: 'Reason' }), 'Superseded by FR-AUTH-0002.');
