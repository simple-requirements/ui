...
const projectRoleSchema = z.enum(['requirements_engineer', 'developer', 'viewer']);
...
export const administratorProjectSummarySchema = z.object({
    ...
    categories: z.array(z.object({ name: z.string(), requirementCount: z.number().int().nonnegative() })),
...
const administratorProjectSummariesSchema = z.array(administratorProjectSummarySchema);
...
export async function listAdministratorProjects(): Promise<AdministratorProjectSummary[]> {
    const response = await adminListProjects();

    return administratorProjectSummariesSchema.parse(response.data);
}

export async function createAdministratorProject(name: string): Promise<AdministratorProjectSummary> {
...
export async function deleteAdministratorProject(projectId: string): Promise<void> {
