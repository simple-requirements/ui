...
function getNextActiveTabId(
  ...
  const closedTabIndex = openTabsBeforeClose.findIndex(
  ...
  const previousTab =
    closedTabIndex > 0 ? remainingTabs.at(closedTabIndex - 1) : undefined;
...
export function openTab(tab: TabBarTab): void {
  tabBarStore.setState((state) => {
    const tabExists = state.openTabs.some(
      (openTabItem) => openTabItem.id === tab.id,
    );
