...
import {
    getRequirementFormKey,
    getRequirementFormTitle,
...
export function useRequirementFormController(
    ...
    const formKey = getRequirementFormKey(mode, projectId, data.requirement?.id ?? requirementId, initialCategoryId);
    ...
    const [formState, formAction, pending] = useRequirementFormAction({
        ...
        onSaved: (savedRequirement) => {
            ...
            showToastMessage(toastMessages.requirementSaved(savedRequirement.visibleKey, mode));

            navigation.allowNavigation();
            openTab({ id: detailsRoute, label: savedRequirement.visibleKey, closable: true });
