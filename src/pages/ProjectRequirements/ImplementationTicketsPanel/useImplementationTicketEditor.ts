...
type UseImplementationTicketEditorArgs = Readonly<{
...
export type ImplementationTicketEditor = Readonly<{
  ...
  valid: boolean;
  updateFormField: (
    field: ImplementationTicketFormField,
    value: string,
  ) => void;
  submit: () => Promise<void>;
...
export function useImplementationTicketEditor({
  ...
  const [form, setForm] = useState<ImplementationTicketFormState>(
    emptyImplementationTicketForm,
  );
