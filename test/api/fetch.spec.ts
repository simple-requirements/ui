...
describe('apiFetch', () => {
    ...
    it('adds the in-memory bearer token to authenticated requests.', async () => {
        ...
        expect(new Headers(requestInit.headers).get('Authorization')).toBe('Bearer opaque-token');
    });
    ...
    it('clears authentication and throws a typed error for 401 responses.', async () => {
        setAuthenticatedSession({ accessToken: 'expired-token', user: authenticatedUser });
        vi.stubGlobal(
            'fetch',
            vi.fn().mockResolvedValue(createJsonResponse({ message: 'Unauthorized' }, { status: 401 })),
        );

        await expect(apiFetch('/projects')).rejects.toMatchObject({ name: 'AuthenticationRequiredError', status: 401 });
    ...
    it('keeps authentication and throws a typed error for 403 responses.', async () => {
        setAuthenticatedSession({ accessToken: 'valid-token', user: authenticatedUser });
        vi.stubGlobal(
            'fetch',
            vi.fn().mockResolvedValue(createJsonResponse({ message: 'Forbidden' }, { status: 403 })),
        );

        await expect(apiFetch('/projects')).rejects.toMatchObject({ name: 'AccessDeniedError', status: 403 });
