...
vi.mock("@/stores/toastStore", () => ({
  showToastMessage: mocks.showToastMessage,
}));
...
function state() {
  return {
    ...
    changeRoleAsync: vi
      .fn()
      .mockResolvedValue({ ...user, status: "deactivated" }),
    changeStatus: vi.fn(),
    changeStatusAsync: vi
      .fn()
      .mockImplementation(({ status }: StatusChange) =>
...
describe("UserAdministrationPage", () => {
  ...
  it("disables self-deactivation and self-session revocation in the user table", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Deactivate Administrator" }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: "Log out Administrator" }),
    ).toBeDisabled();
  });
  ...
  it("blocks the role dialog while the selected user is logged in", async () => {
    ...
    expect(toastMessage).toEqual({
      ...
      detail:
        "Developer is currently logged in and cannot be temporarily deactivated. Log the user out first.",
      life: 5000,
    });
  });
  ...
  it("temporarily deactivates an active logged-out user before opening the role dialog and reactivates on cancel", async () => {
    const administration = state();
    administration.changeStatusAsync.mockImplementation(
      ({ status }: StatusChange) => Promise.resolve({ ...user, status }),
    );
    mocks.useUserAdministration.mockReturnValue(administration);
    mocks.useUserPresence.mockReturnValue(
      new Map([
    ...
    await waitFor(() =>
      expect(administration.changeStatusAsync).toHaveBeenCalledTimes(2),
    );
  ...
  it("saves a role change and restores the temporarily deactivated account", async () => {
    ...
    await waitFor(() =>
      expect(administration.changeRoleAsync).toHaveBeenCalledWith({
  ...
  it("moves selected-user account status administration into the ActionBar", async () => {
    ...
    await waitFor(() =>
      expect(administration.changeStatus).toHaveBeenCalledWith({
        user,
        status: "deactivated",
      }),
    );
