...
type ApiResponse<TData, TStatus extends number = number> = Readonly<{
  data: TData;
  status: TStatus;
  headers: Headers;
}>;
...
export type BootstrapAdministratorRequest = RegisterUserRequest &
  Readonly<{ bootstrapSecret?: string }>;
export type MessageResponse = Readonly<{ message: string }>;
export type BootstrapStatusResponse = Readonly<{
  registrationAvailable: boolean;
}>;
export type UserAdministrationResponse = Readonly<{
  id: string;
  username: string;
  email: string;
  displayName: string;
...
export function login(
  data: LoginRequest,
): Promise<ApiResponse<LoginResponse, 200>> {
...
export function getAuthenticatedUser(
  accessToken?: string,
): Promise<ApiResponse<AuthenticatedUser, 200>> {
...
export function getAuthenticationBootstrapStatus(): Promise<
  ApiResponse<BootstrapStatusResponse, 200>
> {
...
export function listUsers(): Promise<
  ApiResponse<UserAdministrationResponse[], 200>
> {
...
export function listUserSessions(
  userId: string,
): Promise<ApiResponse<SessionResponse[], 200>> {
  return apiFetch(`/admin/users/${encodeURIComponent(userId)}/sessions`, {
    method: "GET",
  });
}
