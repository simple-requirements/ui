...
type RemoveMembershipVariables = Readonly<{
  projectId: string;
  userId: string;
  displayName: string;
}>;
...
export function useProjectMembershipAdministration(
  selectedProjectId: string | undefined,
) {
  const projectsQuery = useQuery({
    queryKey: administrationProjectsQueryKey,
  ...
  const setMembershipMutation = useMutation({
    ...
    onSuccess: async (membership, variables) => {
      ...
      showToastMessage(
        toastMessages.projectMembershipSaved(membership.displayName),
      );
    },
    onError: () =>
      showToastMessage(toastMessages.projectMembershipAdministrationFailed()),
  });
  const removeMembershipMutation = useMutation({
    ...
    onSuccess: async (_response, variables) => {
      ...
      await refreshCurrentUserMemberships(variables.userId);
      showToastMessage(
        toastMessages.projectMembershipRemoved(variables.displayName),
      );
    },
    onError: () =>
      showToastMessage(toastMessages.projectMembershipAdministrationFailed()),
  ...
  return {
    ...
    mutationPending:
      setMembershipMutation.isPending || removeMembershipMutation.isPending,
