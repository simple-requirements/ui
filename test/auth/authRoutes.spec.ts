...
describe('authentication routes', () => {
    it('preserves internal application destinations.', () => {
        expect(getSafeReturnTo({ returnTo: '/projects/one/requirements?filter=open#details' })).toBe(
    ...
    it('builds a relative URL from the current router location.', () => {
        expect(getCurrentRelativeUrl({ pathname: '/projects/one', search: '?tab=details', hash: '#owner' })).toBe(
            '/projects/one?tab=details#owner',
        );
    });
});
