...
When('I reject the requirement because {string}', async ({ page }, reason: string) => {
    await page.getByRole('button', { name: 'Reject' }).click();
    const dialog = page.getByRole('dialog', { name: 'Reject requirement' });
    await expect(dialog.getByRole('textbox', { name: 'Reviewer' })).toHaveCount(0);
    await dialog.getByRole('textbox', { name: 'Reason' }).fill(reason);
    await dialog.getByRole('button', { name: 'OK' }).click();
});

When('I approve the requirement', async ({ page }) => {
...
When('I mark the requirement obsolete because {string}', async ({ page }, reason: string) => {
    await page.getByRole('button', { name: 'Obsolete' }).click();
    const dialog = page.getByRole('dialog', { name: 'Mark requirement obsolete' });
...
Then('the requirement details should show rejection because {string}', async ({ page }, reason: string) => {
...
Then('the requirement status should be {string}', async ({ page }, status: string) => {
    await expect(getRequirementDetailsPanel(page)).toContainText(status);
});

Then('the requirement details should show obsolescence because {string}', async ({ page }, reason: string) => {
    const detailsPanel = getRequirementDetailsPanel(page);
