...
export function ReviewPage() {
    ...
    const decisionRequest = useSelector(actionBarStore, (state) => state.reviewDecisionRequest);
    const [composer, setComposer] = useState<ReviewComposer>();
    const [commentToResolve, setCommentToResolve] = useState<ReviewComment>();
    const { pending, runReviewAction } = useReviewMutationRunner(projectId, requirementId);
    const { requirement, comments, summary, loading, error } = useReviewQueries(projectId, requirementId);
    ...
    useEffect(() => {
        ...
        if (requirement.status !== 'draft') {
            clearReviewDecisionRequest();
            void navigate(getProjectRequirementDetailsRoute(projectId, requirement.id), { replace: true });
        }
    ...
    const activeRequirementId = requirementId;

    async function submitComposer(text: string): Promise<void> {
        const succeeded = await runReviewAction(
            () =>
                composer?.mode === 'reply' ?
                    createReviewReply(activeProjectId, activeRequirementId, composer.comment.id, text)
                :   createReviewComment(activeProjectId, activeRequirementId, text),
            composer?.mode === 'reply' ? toastMessages.reviewReplyCreated() : toastMessages.reviewCommentCreated(),
        );
    ...
    async function resolveComment(): Promise<void> {
        ...
        const succeeded = await runReviewAction(
            () => resolveReviewComment(activeProjectId, activeRequirementId, commentToResolve.id),
    ...
    async function confirmDecision(reason?: string): Promise<void> {
        const succeeded = await runReviewAction(
            async () => {
                const updatedRequirement =
                    decisionRequest === 'reject' ?
                        await rejectReview(activeProjectId, activeRequirementId, reason ?? '')
                    :   await approveReview(activeProjectId, activeRequirementId);
            ...
            decisionRequest === 'reject' ?
                toastMessages.requirementRejected(requirement?.visibleKey ?? 'The requirement')
            :   toastMessages.requirementApproved(requirement?.visibleKey ?? 'The requirement'),
        );

        if (succeeded) {
            clearReviewDecisionRequest();
            void navigate(getProjectRequirementDetailsRoute(activeProjectId, activeRequirementId));
        }
    }
