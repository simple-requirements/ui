...
export function useUserAdministration(selectedUserId: string | undefined) {
    ...
    const statusMutation = useMutation({
        mutationFn: async ({
            user,
            status,
        }: Readonly<{ user: UserAdministrationResponse; status: Exclude<UserStatus, 'pending'> }>) =>
    ...
    const revokeSessionMutation = useMutation({
        mutationFn: ({ userId, sessionId }: Readonly<{ userId: string; sessionId: string }>) =>
            revokeUserSession(userId, sessionId),
        onSuccess: async (_response, variables) => {
            await queryClient.invalidateQueries({ queryKey: userSessionsQueryKey(variables.userId) });
    ...
    return {
        ...
        mutationPending: statusMutation.isPending || revokeSessionMutation.isPending || revokeAllMutation.isPending,
