...
beforeEach(() => {
    mocks.registerUser.mockResolvedValue({ data: { message: 'Registration received.' } });
});
...
describe('RegistrationPage', () => {
    it('submits normalized local-account data and explains the activation steps', async () => {
        ...
        expect(screen.getByRole('status')).toHaveTextContent('Administrator must activate');
    });
    ...
    it('does not expose backend account details when registration fails', async () => {
        ...
        expect(await screen.findByRole('alert')).toHaveTextContent('Registration could not be submitted');
