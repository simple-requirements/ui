...
async function completeForm(): Promise<void> {
  ...
  await user.type(
    screen.getByLabelText("Bootstrap secret"),
    "  setup-secret  ",
  );
}
...
beforeEach(() => {
  mocks.bootstrapAdministrator.mockResolvedValue({
    data: { message: "Bootstrap registration received." },
  });
});
...
describe("BootstrapAdministratorPage", () => {
  ...
  it("switches to normal login when another request completed bootstrap first.", async () => {
    mocks.bootstrapAdministrator.mockRejectedValue(
  ...
  it("shows a specific retry-later message for rate limiting.", async () => {
    mocks.bootstrapAdministrator.mockRejectedValue(
      new ApiError("Rate limited", 429, undefined),
    );
    const user = userEvent.setup();
    renderPage();
    await completeForm();
