...
const mocks = vi.hoisted(() => ({
  useProjectMembershipAdministration: vi.fn(),
}));

vi.mock("@/pages/Administration/useProjectMembershipAdministration", () => ({
  useProjectMembershipAdministration:
    mocks.useProjectMembershipAdministration,
}));
...
describe("ProjectMembershipAdministrationPage", () => {
  it("lets the Administrator select a project before showing memberships", async () => {
    mocks.useProjectMembershipAdministration.mockImplementation(
    ...
    expect(
      screen.getByText(/Select a project to view and manage/iu),
    ).toBeInTheDocument();
    await interaction.selectOptions(screen.getByLabelText("Project"), projectId);

    await waitFor(() =>
      expect(mocks.useProjectMembershipAdministration).toHaveBeenLastCalledWith(
        projectId,
      ),
    );
    expect(
    ...
    expect(
      screen.getByRole("link", { name: "Users and sessions" }),
    ).toHaveAttribute("href", "/administration/users");
  });
});
