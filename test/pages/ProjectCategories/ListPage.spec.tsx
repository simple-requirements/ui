...
const mocks = vi.hoisted(() => ({
  useLiveQuery: vi.fn(),
  getProjectCategoriesCollection: vi.fn((projectId: string) => ({
    id: `categories:${projectId}`,
  })),
  getProjectRequirementsCollection: vi.fn((projectId: string) => ({
    id: `requirements:${projectId}`,
  })),
  deleteProjectCategoryRequest: vi.fn(),
  getListProjectCategoriesQueryKey: vi.fn(
    (projectId: string) => [`/projects/${projectId}/categories`] as const,
  ),
...
describe("ProjectCategories ListPage", () => {
  it("renders category keys as copy links.", () => {
    ...
    expect(
      screen.getByRole("link", { name: /copy category key auth/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /copy category key perf/i }),
    ).toBeInTheDocument();
  });
  ...
  it("renders requirement counts calculated from loaded project requirements.", () => {
    ...
    mocks.useLiveQuery.mockImplementation(
      (_queryBuilder, dependencies: readonly unknown[]) => {
  ...
  it("deletes the selected category from the context menu when it has no requirements.", async () => {
    ...
    expect(mocks.invalidateQueries).toHaveBeenCalledWith({
      queryKey: ["/projects/project-alpha/categories"],
    });
    expect(mocks.showToastMessage).toHaveBeenCalledWith({
      severity: "success",
