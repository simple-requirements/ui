...
beforeEach(() => {
  mocks.getAuthenticationBootstrapStatus.mockResolvedValue({
    data: { registrationAvailable: false },
  });
});
...
describe("AuthenticationEntryPage", () => {
  ...
  it("does not expose login when bootstrap status cannot be established and supports retry.", async () => {
    mocks.getAuthenticationBootstrapStatus.mockRejectedValueOnce(
    ...
    await waitFor(() =>
      expect(mocks.getAuthenticationBootstrapStatus).toHaveBeenCalledTimes(2),
    );
    expect(
      await screen.findByRole("heading", { name: "Sign in" }),
    ).toBeInTheDocument();
