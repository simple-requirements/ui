...
vi.mock("@/components/ContextMenu/AppContextMenu", () => ({
  AppContextMenu: ({
    ...
    model: readonly { label?: string; command?: () => void }[];
  }) => (
    <nav aria-label="Sidebar context menu">
      {model.map((item) => (
        <button key={item.label} type="button" onClick={item.command}>
          {item.label}
        </button>
      ))}
    </nav>
...
describe("SidebarContextMenu", () => {
  it("contains only project-workspace export actions.", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Export all projects" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /rename project/i }),
