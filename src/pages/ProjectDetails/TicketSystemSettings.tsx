...
export function TicketSystemSettings({ project }: Readonly<{ project: Project }>) {
  ...
  async function save(): Promise<void> {
    ...
    try {
      ...
      showToastMessage(toastMessages.implementationActionFailed(error instanceof Error ? error.message : "The settings could not be saved."));
    } finally { setPending(false); }
  }
  return (
    <section className="project-details-page__ticket-settings" aria-labelledby="ticket-system-settings-title">
      <h2 id="ticket-system-settings-title">Ticket system</h2>
      <label htmlFor="ticket-url-template">Ticket URL template</label>
      <div>
        <InputText id="ticket-url-template" value={value} placeholder="https://github.com/xxx/{ticket-id}" onChange={(event) => setValue(event.currentTarget.value)} />
        <Button type="button" label="Save" loading={pending} onClick={() => void save()} />
      </div>
