...
import {
  ...
  type ReviewActionRequirement,
} from "@/stores/actionBarStore";
import { showToastMessage } from "@/stores/toastStore";

export function useObsoleteRequirementAction(
  ...
  async function confirm(reason?: string): Promise<void> {
    ...
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: getListProjectRequirementsQueryKey(
          updatedRequirement.projectId,
        ),
      });
      setVisible(false);
      showToastMessage(
        toastMessages.requirementObsolete(updatedRequirement.visibleKey),
      );
    } catch (error) {
      showToastMessage(
