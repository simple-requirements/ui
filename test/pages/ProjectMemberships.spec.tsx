...
describe("ProjectMemberships", () => {
  it("adds memberships, changes multiple roles, and removes memberships", async () => {
    ...
    expect(within(aliceRow).getAllByRole("cell")[0]).toHaveTextContent(
      "Viewer",
    );

    const aliceRoles = within(aliceRow).getByRole("group", {
      name: "Roles for Alice Member",
    });
    await interaction.click(within(aliceRoles).getByLabelText("Developer"));
    await interaction.click(
      within(aliceRow).getByRole("button", { name: "Save roles" }),
    );

    expect(onSetMembership).toHaveBeenCalledWith(
      alice.id,
      alice.displayName,
      ["developer", "viewer"],
    );

    const assignment = screen.getByRole("form", {
      name: "Assign project membership",
    });
    await interaction.selectOptions(
      within(assignment).getByLabelText("User"),
      bob.id,
    );
    const newRoles = within(assignment).getByRole("group", {
      name: "Roles for new membership",
    });
    await interaction.click(
      within(newRoles).getByLabelText("Requirements Engineer"),
    );
    await interaction.click(within(newRoles).getByLabelText("Developer"));
    await interaction.click(
      within(assignment).getByRole("button", { name: "Add membership" }),
    );

    expect(onSetMembership).toHaveBeenLastCalledWith(
      bob.id,
      bob.displayName,
      ["requirements_engineer", "developer"],
    );

    await interaction.click(
      within(aliceRow).getByRole("button", { name: "Remove membership" }),
    );
    expect(onRemoveMembership).toHaveBeenCalledWith(
      alice.id,
      alice.displayName,
    );
  });
