...
function user(
    ...
    return {
        ...
        status: 'active',
        globalRoles,
        ...(roles === undefined ? {} : { projectMemberships: [{ projectId: 'project-alpha', roles }] }),
    };
}
...
describe('project permissions', () => {
    ...
    it('grants ticket management to developers without requirement management', () => {
        ...
        expect(hasProjectRole(user(['developer']), 'project-alpha', 'developer')).toBe(true);
        expect(permissions.canReadProject).toBe(true);
        expect(permissions.canManageRequirements).toBe(false);
        expect(permissions.canManageTickets).toBe(true);
    });
    ...
    it('grants requirement and ticket management to requirements engineers', () => {
        const permissions = getProjectPermissions(user(['requirements_engineer']), 'project-alpha');

        expect(
            hasAnyProjectRole(user(['requirements_engineer']), 'project-alpha', ['requirements_engineer', 'developer']),
        ).toBe(true);
    ...
    it('grants administrators project administration and read access', () => {
        const permissions = getProjectPermissions(user([], ['administrator']), 'project-alpha');

        expect(hasProjectPermission(permissions, projectPermissionKinds.read)).toBe(true);
