...
import type { AccountRole, AuthenticatedUser, ProjectRole } from '@/auth/authTypes';
...
describe('project permissions', () => {
    it('denies project access without membership', () => {
        const permissions = getProjectPermissions(user('requirements_engineer', false), 'project-alpha');
    ...
    it('grants requirement and ticket management to requirements engineers', () => {
        ...
        const acceptedRoles: readonly ProjectRole[] = ['requirements_engineer', 'developer'];
        const permissions = getProjectPermissions(requirementsEngineer, 'project-alpha');

        expect(hasAnyProjectRole(requirementsEngineer, 'project-alpha', acceptedRoles)).toBe(true);
        expect(hasProjectPermission(permissions, projectPermissionKinds.manageRequirements)).toBe(true);
        expect(permissions.canManageTickets).toBe(true);
    });

    it('denies Administrators all project-content permissions', () => {
        const permissions = getProjectPermissions(user('administrator', false), 'project-alpha');

        expect(hasProjectPermission(permissions, projectPermissionKinds.read)).toBe(false);
        expect(permissions.canManageRequirements).toBe(false);
        expect(permissions.canManageTickets).toBe(false);
    });
});
