...
type Options = Readonly<{
    ...
    setSelectedCategoryId: (updater: string | undefined | ((id: string | undefined) => string | undefined)) => void;
    onDeleted: () => void;
}>;

export function useDeleteCategory({ projectId, setSelectedCategoryId, onDeleted }: Options) {
    ...
    async function confirm(): Promise<void> {
        ...
        try {
            ...
            setSelectedCategoryId((current) => (current === category.id ? undefined : current));
    ...
    return { candidate, pending, request, abort: () => setCandidate(undefined), confirm };
}
