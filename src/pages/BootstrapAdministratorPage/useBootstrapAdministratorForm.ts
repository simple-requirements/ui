...
export function useBootstrapAdministratorForm({
  onBootstrapUnavailable,
}: Options) {
  const [form, setForm] = useState<BootstrapAdministratorFormState>(
    emptyBootstrapAdministratorForm,
  );
  const [pending, setPending] = useState(false);
  const [submitted, setSubmitted] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string>();
  const { passwordsMatch, canSubmit } =
    getBootstrapAdministratorFormValidation(form);
