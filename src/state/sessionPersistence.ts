import { initialWorkspaceState, type Module, type WorkspaceState } from '@/state/workspaceReducer';
export const WORKSPACE_STATE_KEY = 'requirements-app.backend-workspace.v1';

function loadModule(value: unknown): Module {
    return value === 'categories' ? 'categories' : 'requirements';
}
...
export function loadWorkspaceState(): WorkspaceState {
    try {
        ...
        if (typeof p.activeAppTabId === 'string' && Array.isArray(p.openRequirementTabs))
            return { ...initialWorkspaceState, ...p, activeModule: loadModule(p.activeModule) };
    } catch {
        /* reset */
    }
